...
# =========================================================================
# [ CRITICAL SETUP ]
# 1. Copy 'StcPython.py' from your Spirent API/Python folder to this folder.
# 2. Update the directory below to match your Spirent TestCenter installation.
# =========================================================================
STC_INSTALL_DIR = r"C:\Program Files\Spirent Communications\Spirent TestCenter 5.30\Spirent TestCenter Application"

# Set the system environment variable as required by Spirent TestCenter
os.environ['STC_PRIVATE_INSTALL_DIR'] = STC_INSTALL_DIR

# Ensure current directory is in sys.path so it finds the copied StcPython.py
...
try:
    ...
    print_log("Please copy StcPython.py to the AI_Automation-Tool folder.")
...
def run_stc_traffic(chassis_ip, tx_port_loc, rx_port_loc, vlan_id, frame_size, load_percent, duration):
    ...
    try:
        # Verify Setup per Spirent Manual
        ...
        if tx_count > 0 and rx_count >= (tx_count * 0.99):
            print_log("[STC_TEST_SUCCESS] Traffic test passed criteria.")
            return True
        else:
            print_log("[STC_TEST_FAILED] High frame drop or no traffic generated.")
            return False
